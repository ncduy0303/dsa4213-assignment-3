...
class DataTrainingArguments:
    ...
    do_regression: Optional[bool] = field(
        default=None,
        metadata={
    ...
    def __post_init__(self):
        if self.dataset_name is None:
            if self.train_file is None or self.validation_file is None:
                raise ValueError(
                    " training/validation file or a dataset name.")
...
def get_label_list(raw_dataset, split="train") -> list[str]:
    ...
    if isinstance(raw_dataset[split]["label"][0], list):
        label_list = [label for sample in raw_dataset[split]["label"]
                      for label in sample]
...
def main():
    ...
    # Create unique output directory for each W&B run
    if "wandb" in training_args.report_to and os.environ.get("WANDB_RUN_ID"):
        training_args.output_dir = os.path.join(
            training_args.output_dir, os.environ["WANDB_RUN_ID"])
        logger.info(
            f"W&B run detected. Setting output directory to: {training_args.output_dir}")
    ...
    predict_dataset: Optional[datasets.Dataset] = None

    if not isinstance(raw_datasets, DatasetDict):
        raise TypeError(
            "Expected a dataset with named splits (DatasetDict). "
            "Pass multiple splits (train/validation[/test]) or a dataset with predefined splits."
        )
    ...
    if data_args.validation_split_name is not None:
        logger.info(
            f"using {data_args.validation_split_name} as validation set")
    ...
    if is_regression:
        ...
        if num_labels <= 1:
            raise ValueError(
                "You need more than one label to do classification.")
    ...
    if training_args.do_train and not is_regression:  # classification, training
        ...
        model.config.id2label = {
            id: label for label, id in label_to_id.items()}
    ...
    def multi_labels_to_ids(labels: list[str]) -> list[float]:
        """Convert multi-labels to multi-hot encoding"""
    ...
    def preprocess_function(examples):
        ...
        # Tokenize the texts
        result = tokenizer(
            examples["sentence"], padding=padding, max_length=max_seq_length, truncation=True)
        if label_to_id is not None and "label" in examples:
            if is_multi_label:
                result["label"] = [multi_labels_to_ids(
    ...
    if data_args.metric_name is not None:
        metric = (
            evaluate.load(data_args.metric_name,
                          config_name="multilabel", cache_dir=model_args.cache_dir)
            if is_multi_label
        ...
        if is_regression:
            ...
            if is_multi_label:
                ...
                metric = evaluate.load(
                    "accuracy", cache_dir=model_args.cache_dir)
                logger.info(
